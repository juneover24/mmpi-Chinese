...
function score() {
  ...
  for (i = 0; i < scales.length; ++i) {
    ...
    if (tscale !== undefined) {
      ...
      percent = (n * 100) / (scales[i][1].length + scales[i][2].length);
  ...
  append_result_text("答案摘要：");
  s = "";
  for (q = 1; q < questions.length; ++q) {
    s += ans[q] || "?";
    if (s.length >= 75) {
      append_result_text(s);
      s = "";
    }
  }
  if (s.length) {
...
function saveAsPDF() {
  var docbody = document.getElementsByTagName("body")[0];
  var pdf = new jsPDF();
  pdf.fromHTML(docbody, 15, 15, { width: 170 });
  pdf.save("mmpi-2-results.pdf");
}

// Score the test using a text string
function score_text(anstext) {
  var i, j;
  var t, f, cs;
  var n = longform ? questions.length : 371;

  // Get the answer character
  for (i = 1, j = 0; i < n; ++i) {
    switch (anstext.charAt(j++)) {
      case "t":
      case "T":
      case "1":
        ans[i] = "T";
...
// Write a question with radio buttons
function doc_write_question(name, text) {
  var qid = "q" + name;
  document.write('<div class="question-item" id="' + qid + '">');
  document.write('<label><input type="radio" name="' + name + '" value="T" />是</label>');
  document.write('<label><input type="radio" name="' + name + '" value="F" />否</label>');
  document.write('<label><input type="radio" name="' + name + '" value="?" checked />无法确定</label>');
  document.write(" " + text);
  document.write('</div>');
}
...
function updatePaginationControls() {
  ...
  if (!paginationContainer) {
    ...
    var questionsContent = document.getElementById('questions-content');
...
function showQuestionsForPage(page) {
  ...
  var endIndex = Math.min(startIndex + questionsPerPage, allQuestions.length);
...
function setupAutoScroll() {
  // 获取所有单选按钮
  var radioButtons = document.querySelectorAll('#questions-content input[type="radio"]');
  
  // 为每个单选按钮添加change事件
  for (var i = 0; i < radioButtons.length; i++) {
    radioButtons[i].addEventListener('change', function() {
      // 找到当前问题
      var currentQuestion = this.closest('.question-item, .re-question');
...
// 修改doc_write_all_questions函数，使用DOM API代替document.write
function doc_write_all_questions() {
  ...
  if (re_scale_only) {
    ...
    // 显示普通问题
    for (var i = 1; i < n; ++i) {
      add_question_to_element(questionsContent, i, i + ". " + questions[i]);
    }
  }
...
// 提示用户发送邮件
function mail() {
  alert("请先点击\"导出PDF\"按钮生成PDF文件，然后将该PDF文件作为附件发送至gushao@gushao.club，我们将在三天内回复分析结果。");
}
...
function updateProgressBar() {
  ...
  if (re_scale_only) {
    ...
    if (re_index !== -1) {
      ...
      totalQuestions = re_qs_true.length + re_qs_false.length;
      ...
      for (var i = 0; i < re_qs.length; i++) {
        ...
        totalQuestions = 0; // RE scale not found
    }
  } else {
    totalQuestions = longform ? questions.length - 1 : 370;
    var limit = totalQuestions + 1; // questions are 1-indexed
  ...
  var percentage = totalQuestions > 0 ? Math.floor((answeredQuestions / totalQuestions) * 100) : 0;
  ...
  progressText.textContent = '已完成: ' + answeredQuestions + ' / ' + totalQuestions + ' (' + percentage + '%)';
  
  console.log("进度更新: " + answeredQuestions + "/" + totalQuestions + " (" + percentage + "%)");
...
// 向指定元素添加问题
function add_question_to_element(element, name, text, isReQuestion) {
  var qid = "q" + name;
  ...
  falseLabel.className = 'option-label false-option';
  var falseInput = document.createElement('input');
  falseInput.type = 'radio';
  falseInput.name = name;
  falseInput.value = 'F';
  falseInput.className = 'option-input';
  
  var falseSpan = document.createElement('span');
  falseSpan.className = 'option-text';
  falseSpan.textContent = '否';
  ...
  // 创建"无法确定"选项
  var uncertainLabel = document.createElement('label');
  uncertainLabel.className = 'option-label uncertain-option';
  var uncertainInput = document.createElement('input');
  uncertainInput.type = 'radio';
  uncertainInput.name = name;
  uncertainInput.value = '?';
  uncertainInput.className = 'option-input';
  ...
  // 初始不选择任何选项，而不是默认选择"无法确定"
  ans[parseInt(name, 10)] = undefined;
  
  var uncertainSpan = document.createElement('span');
  uncertainSpan.className = 'option-text';
  uncertainSpan.textContent = '无法确定';
  
  uncertainLabel.appendChild(uncertainInput);
  uncertainLabel.appendChild(uncertainSpan);
  optionsContainer.appendChild(uncertainLabel);
  
  // 添加事件监听器，高亮选中的选项
  var inputs = [trueInput, falseInput, uncertainInput];
  for (var i = 0; i < inputs.length; i++) {
    inputs[i].addEventListener('change', function() {
      // 移除所有选项的active类
      var labels = this.closest('.options-container').querySelectorAll('.option-label');
      ...
      if (this.checked) {
        ...
        // 更新ans数组
        var questionNumber = parseInt(this.name, 10);
        // 确保ans数组有足够大小
        while(ans.length <= questionNumber) {
          ans.push(undefined);
        }
        ans[questionNumber] = this.value;
        console.log("问题" + questionNumber + "的答案更新为: " + this.value);
      }
...
function updateSubmitButtonState() {
  ...
  var promptText = document.getElementById('submit-prompt');
  ...
  if (percentage === 100) {
    ...
    // 还有未回答的问题
    submitButton.disabled = true; // 在所有问题回答完毕前禁用按钮
    submitButton.textContent = '请先完成所有问题'; // 更改按钮文本以提示用户
...
function checkIncompleteQuestions() {
  ...
  // 获取所有问题
  var allQuestions = document.querySelectorAll('.question-item, .re-question');
...
function addQuickNavigation(skipProgressUpdate) {
  ...
  navControls.appendChild(pageNav);
